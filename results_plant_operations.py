...
def draw_plant_architecture_plotly(topology, ram_params=None):
    ...
    _C = {"ez": "#16213e", "stack": "#0f3460", "comp": "#533483",
          "fill": "#e94560", "aux": "#607D8B",
          "arrow": "#a0a0c0", "header": "#c0c0d8", "label": "#e0e0f0"}
    ...
    def _circle(cx, cy, r, label=None, hover=None):
        shapes.append(dict(
            type="circle", x0=cx-r, y0=cy-r, x1=cx+r, y1=cy+r,
            fillcolor="rgba(0,0,0,0)",
            line=dict(color=_C["aux"], width=1.5, dash="dash"),
        ))
        if label:
            anns.append(dict(x=cx, y=cy, text=f"<b>{label}</b>", showarrow=False,
                            font=dict(size=7, color=_C["aux"])))
        hx.append(cx); hy.append(cy); ht.append(hover or label or "")
    ...
    def _draw_block(x0, x1, n_ez, n_comp, n_fill, stacks, ez_rate, comp_rate,
        ...
        y_ez   = Y_TOP
        y_stk  = Y_TOP - th * 0.12
        y_eaux = y_stk - th * 0.09
        y_hdr  = y_eaux - th * 0.07
        y_comp = y_hdr - th * 0.10
        y_caux = y_comp - th * 0.09
        y_fill = y_caux - th * 0.10
        AUX_R  = 0.018
        ...
        for i, xez in enumerate(x_ez):
            ...
            if stacks >= 2:
                ...
                _hl(sm, xa, xb)
                _vl(xez, sm, y_eaux + AUX_R)
            else:
                _box(xez, y_stk, SBW, SBH, _C["stack"], "Stk",
                     hover=f"<b>Stack (EZ {i+1})</b><br>{stk_hov}")
                _vl(xez, y_stk - SBH/2, y_eaux + AUX_R)
            _circle(xez, y_eaux, AUX_R, "Aux",
                    hover=f"<b>EZ {i+1} Aux</b><br>Exponential (BOM rollup)<br>"
                          f"Equipment between EZ and compressor")
            _vl(xez, y_eaux - AUX_R, y_hdr)
        ...
        for j, xc in enumerate(x_comp):
            ...
            _vl(xc, y_comp - BH*0.53, y_caux + AUX_R)
            _circle(xc, y_caux, AUX_R, "Aux",
                    hover=f"<b>Comp {j+1} Aux</b><br>Exponential (BOM rollup)<br>"
                          f"Equipment between compressor and fill lines")
        ...
        if dedicated_lines:
            ...
            for j, xc in enumerate(x_comp):
                ...
                hdr_y = y_caux - AUX_R - (y_caux - AUX_R - y_fill)*0.25
                _hl(hdr_y, x_fl[0], x_fl[-1], lw=1.2)
                _vl(xc, y_caux - AUX_R, hdr_y)
    ...
    for k, (lbl, col) in enumerate(legend_items):
        lx = 0.02 + k * 0.20
        shapes.append(dict(type="rect", x0=lx, y0=0.96, x1=lx+0.18, y1=0.99,
                          fillcolor=col, line=dict(color="white", width=0.5)))
        anns.append(dict(x=lx+0.09, y=0.975, text=f"<b>{lbl}</b>", showarrow=False,
                        font=dict(size=8, color="white")))
    lx_aux = 0.02 + 4 * 0.20
    shapes.append(dict(type="circle", x0=lx_aux, y0=0.961, x1=lx_aux+0.028, y1=0.989,
                      fillcolor="rgba(0,0,0,0)", line=dict(color=_C["aux"], width=1.5, dash="dash")))
    anns.append(dict(x=lx_aux+0.045, y=0.975, text=f"<b>Aux node (Exp)</b>", showarrow=False,
                    font=dict(size=8, color=_C["aux"]), xanchor="left"))
    ...
    anns.append(dict(x=0.5, y=0.23, text=f"<i>{footer}</i>", showarrow=False,
    ...
    fig.update_layout(
        ...
        yaxis=dict(range=[0.20, 1.03], showgrid=False, zeroline=False, showticklabels=False, fixedrange=True),
